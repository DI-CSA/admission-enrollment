...
const STAGE_INSCRITO = process.env.RD_CRM_DEAL_STAGE_ID?.trim() || "6a4bb5634eabe9001d666600";
const STAGE_PAGO = process.env.RD_CRM_DEAL_STAGE_PAGO_ID?.trim() || "6a4bb564aa4cb20022b93a3c";
const PRODUTO_TAXA = process.env.RD_CRM_PRODUCT_TAXA_ID?.trim() || "6a46b79dc5417b0024bd0171";
...
async function listarDeals() {
  ...
  for (let page = 1; page <= 100; page++) {
    ...
    for (const d of lista) {
      ...
      out.push({ id: sid(d), stage: sid(d.deal_stage), idLan: m ? m[1] : null });
    }
...
async function garantirValorTaxa(dealId) {
  ...
  const temTaxa = produtos.some((p) => p.product_id === PRODUTO_TAXA || (p.name ?? "").trim() === NOME_TAXA);
  if (!temTaxa) {
    await rd("POST", `/deals/${dealId}/deal_products`, {
      deal_product: { product_id: PRODUTO_TAXA, name: NOME_TAXA, base_price: VALOR_TAXA, price: VALOR_TAXA, amount: 1, recurrence: "spare" },
    });
  }
}
...
async function eventoPagamentoConfirmado(insc) {
  ...
  const payload = {
    ...
    payload: {
      ...
      ...(insc.dataPagamento ? { cf_data_pagamento_taxa: insc.dataPagamento } : {}),
    },
  };
  await fetch(`https://api.rd.services/platform/conversions?api_key=${encodeURIComponent(MKT_TOKEN)}`, {
...
SELECT i.NUMEROINSCRICAO, i.IDPS, i.IDLAN,
 ...
 ORDER BY i.NUMEROINSCRICAO`);
  ...
  return recordset.map((r) => ({
    numeroInscricao: r.NUMEROINSCRICAO, idps: r.IDPS, idLan: r.IDLAN ?? null,
...
// ---- Execução --------------------------------------------------------------
console.log(`>>> Conciliação INSCRIÇÃO (taxa R$200) — LOCAL — ano ~%${ANO}% — ${COMMIT ? "COMMIT" : "DRY-RUN"}`);
...
console.log(`RM: ${pagas.length} inscrição(ões) com taxa PAGA | RD: ${deals.length} deal(s) (${porIdLan.size} com [LAN:]).`);

let movidas = 0, ja = 0, semDeal = 0, semIdLan = 0, falhas = 0;
for (const insc of pagas) {
  if (insc.idLan == null) { semIdLan++; continue; }
  const deal = porIdLan.get(String(insc.idLan));
  if (!deal) { semDeal++; continue; }
  // Forward-only: só move quem está em "Inscrito".
  if (deal.stage !== STAGE_INSCRITO) { ja++; continue; }
  if (!COMMIT) { movidas++; console.log(`  [DRY] insc ${insc.numeroInscricao} -> Taxa paga`); continue; }
  const ok = await rd("PUT", `/deals/${deal.id}`, { deal: { deal_stage_id: STAGE_PAGO } });
  if (!ok.ok) { falhas++; continue; }
  movidas++;
  await garantirValorTaxa(deal.id);
  await eventoPagamentoConfirmado(insc);
}
console.log("\n=== RESULTADO ===");
console.log({ verificadas: pagas.length, movidas, jaAvancadas: ja, semDeal, semIdLan, falhas });
